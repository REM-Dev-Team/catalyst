...
function MakeswiftYouTubeVideoCarouselRow({
  ...
  const columnsNumber = parseInt(columnsPerRow, 10) || 3;
  const columnsNumberMobile = parseInt(columnsPerRowMobile, 10) || 1;
  const [activePlaylist, setActivePlaylist] = useState<string | undefined>(playlist1Id);

  // Type guard for playlist detail - uses Reflect.get to avoid type assertions
  function isPlaylistDetail(detail: unknown): detail is { playlistId?: string } {
  ...
  // Convert error to string for display - error is string | null from useYouTubeVideos
  const errorMessage: string | null = typeof error === 'string' ? error : null;
  ...
  if (errorMessage) {
    return (
      <div className={clsx('space-y-6', className)}>
        {renderHeader()}
        <div className="rounded-xl bg-gray-50 p-6 text-center" style={{ color: textColor }}>
    ...
    <div className={clsx('space-y-6', className)}>
      ...
      {wrapToRows ? (
        ...
        <>
          ...
          `}</style>
          <div className="youtube-grid-responsive grid gap-4 gap-y-8">
            {videos.map((video) => (
              <YouTubeVideoCard
                key={video.id}
                video={video}
